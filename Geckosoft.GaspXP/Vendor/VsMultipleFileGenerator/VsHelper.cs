...
namespace Geckosoft.GaspXP.Vendor.VsMultipleFileGenerator
{
    public static class VsHelper
    {
        ...
        {
            var vs = (DTE)provider.GetService(typeof(DTE)); if (vs == null) throw new InvalidOperationException("DTE not found."); return ToHierarchy(vs.SelectedItems.Item(1).ProjectItem.ContainingProject);
        }
            ...
            using (var projectReader = XmlReader.Create(project.FileName))
            {
            	...
            	if (projectReader.NameTable != null)
            	{
            		...
            		while (projectReader.Read())
            		{
            			...
            			{
            				projectGuid = projectReader.ReadElementContentAsString(); break;
            			}
            		}
            	}
            }
